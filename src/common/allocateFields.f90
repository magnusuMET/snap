...
module allocateFieldsML
  ...
  USE snapfldML, only: u1, u2, v1, v2, w1, w2, bl1, bl2, t1, t2, &
      ...
      max_aircraft_doserate, max_aircraft_doserate_scratch, max_aircraft_scratch, &
      t1_abs, t2_abs, &
...
! the plumes
  ...
  if (compute_max_aircraft_doserate) then
    allocate(max_aircraft_doserate_scratch(nx,ny,nk), &
      max_aircraft_doserate(nx,ny), &
      max_aircraft_scratch(nx,ny,3), &
